    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <div className="card-glow rounded-3xl border border-[#172033]/8 bg-white p-5">
          <p className="text-sm font-semibold text-[#172033]">高风险事件趋势</p>
          <p className="mt-1 text-xs text-[#5d6b82]">过去一周，系统每天筛出的高风险事件数量</p>
          <div className="mt-4 h-64">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={stats.weeklyCriticalTrend}>
                ...
                <Tooltip contentStyle={{ background: '#fff', borderRadius: 16, border: '1px solid rgba(23,32,51,0.08)', boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }} />
        ...
        <div className="card-glow rounded-3xl border border-[#172033]/8 bg-white p-5">
          <p className="text-sm font-semibold text-[#172033]">风险类型分布</p>
          <p className="mt-1 text-xs text-[#5d6b82]">今天检测到的各类风险事件数量</p>
          <div className="mt-4 h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats.riskTypeDistribution}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(23,32,51,0.06)" />
                <XAxis dataKey="name" stroke="rgba(93,107,130,0.5)" tickLine={false} axisLine={false} fontSize={12} />
                <YAxis stroke="rgba(93,107,130,0.5)" tickLine={false} axisLine={false} />
                <Tooltip contentStyle={{ background: '#fff', borderRadius: 16, border: '1px solid rgba(23,32,51,0.08)', boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }} />
      ...
      <div className="card-glow rounded-3xl border border-[#172033]/8 bg-white p-5">
        <p className="text-sm font-semibold text-[#172033]">不同区域风险热度排名</p>
        <p className="mt-1 text-xs text-[#5d6b82]">热度越高，说明该区域发生风险事件越频繁，需要重点关注</p>
        <div className="mt-4 grid gap-3 sm:grid-cols-5">
          {stats.zoneHotspots.map((item, index) => (
            <div key={item.zone} className="rounded-2xl border border-[#172033]/8 bg-[#f5f7fb] p-4">
              <p className="text-xs text-[#5d6b82]/50">第 {index + 1} 名</p>
              <p className="mt-2 text-base font-semibold text-[#172033]">{item.zone}</p>
              <div className="mt-2 h-1.5 rounded-full bg-[#e8edf5]">
      ...
      <div className="card-glow rounded-3xl border border-[#172033]/8 bg-white p-5">
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-semibold text-[#172033]">今日安全巡检概况</p>
            </div>
            <p className="mt-1 text-xs text-[#5d6b82]">系统综合摄像头、床垫、门磁和毫米波设备给出的值班判断</p>
          </div>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {prediction.fusionRisk.modalities.map((m, i) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-xs text-[#5d6b82]">{m.label}</span>
              ...
              <p className="mt-2 text-2xl font-bold text-[#172033]">{m.score}<span className="text-xs font-normal text-[#5d6b82]/50"> 分</span></p>
              <p className="mt-1 truncate text-[11px] text-[#5d6b82]/60">{m.detail}</p>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="card-glow rounded-3xl border border-[#172033]/8 bg-white p-5">
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-semibold text-[#172033]">下一轮重点巡查建议</p>
            </div>
            <p className="mt-1 text-xs text-[#5d6b82]">根据近期事件、老人画像和设备状态自动生成</p>
          </div>
        ...
        <div className="mt-4 space-y-2">
          {prediction.forecast30min.highRiskRooms.slice(0, 3).map((room, i) => (
            ...
            >
              <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-white text-sm font-bold text-[#172033]">{i + 1}</div>
              <div className="min-w-0 flex-1">
                <span className="text-sm font-medium text-[#172033]">{room.roomName}</span>
                <p className="truncate text-xs text-[#5d6b82]/70">{room.reasons[0]}</p>
              </div>
