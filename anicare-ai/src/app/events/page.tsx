  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        {[
          { label: '事件总数', value: mockEvents.length, icon: 'mdi:archive-clock-outline', tone: 'text-[#1a1615]' },
          ...
          <div key={item.label} className="card-glow rounded-2xl border border-[#1a1615]/8 bg-white p-4">
        ...
        {filtered.map((event) => (
          <div key={event.id} className="card-glow rounded-3xl border border-[#1a1615]/8 bg-white p-5 transition-colors hover:border-teal-500/25">
            ...
            <div className="mt-3 grid gap-2 md:grid-cols-3">
              <div className="rounded-2xl bg-[#f8f5f0] px-3 py-2">
                <p className="text-[11px] text-[#5c524a]/50">研判依据</p>
                <p className="mt-1 truncate text-xs font-medium text-[#1a1615]">{event.decision?.cause ?? '视觉识别与护理规则联合判断'}</p>
              </div>
              <div className="rounded-2xl bg-[#f8f5f0] px-3 py-2">
                <p className="text-[11px] text-[#5c524a]/50">建议动作</p>
                <p className="mt-1 truncate text-xs font-medium text-[#1a1615]">{event.decision?.suggestion ?? '安排护理员复核现场'}</p>
              </div>
              <div className="rounded-2xl bg-[#f8f5f0] px-3 py-2">
                <p className="text-[11px] text-[#5c524a]/50">闭环状态</p>
                <p className="mt-1 text-xs font-medium text-[#1a1615]">识别 → 研判 → 派单 → 归档</p>
              </div>
            </div>
