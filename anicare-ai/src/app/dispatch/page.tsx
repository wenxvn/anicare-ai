...
type DispatchStatus = '待指派' | '处理中' | '已完成';
...
const initialQueue: DispatchRow[] = [
  ...
  },
  {
    ...
    zone: 'A栋-4层-房间405', waitMinutes: 12, residentName: '孙丽芳', priority: 8, priorityScore: 61,
    reason: '今日活动量低于个人 7 日均值 45%，且情绪记录显示不愿交流。',
    status: '待指派', time: '2025-05-01 17:20:12',
  },
  {
    id: 'dsp-009', eventId: 'evt-20250501-011', type: '门磁异常开启', risk: '低风险', riskLevel: 'low',
    zone: 'A栋-2层-活动室', waitMinutes: 6, residentName: '周志明', priority: 9, priorityScore: 43,
    reason: '活动室门磁在非开放时段被连续触发，需确认是否为护理员巡查或老人误入。',
    status: '已完成', assignee: '王美华', time: '2025-05-01 18:05:40',
  },
];
      ...
      if (items.length) {
        setQueue((prev) => {
          const normalized = items.slice(0, 8).map((item) => ({
            ...item,
            status: item.status === '已完成' || item.status === '处理中' ? item.status : '待指派',
          })) as DispatchRow[];
      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-relaxed text-[#5d6b82]">
            按事件优先级组织派单、到场和归档，值班人员可以快速处理当前最紧急的工单。
          </p>
        </div>
            ...
            {sortedQueue.map((item, index) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`justify-self-center text-center ${item.waitMinutes > 10 ? 'font-semibold text-red-600' : 'text-[#172033]'}`}>{item.waitMinutes} 分钟</span>
                  ...
                  <span className="justify-self-center">
                    {item.status === '待指派' ? (
                      <button
                        ...
                        className="inline-flex min-w-[64px] justify-center rounded-lg bg-teal-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-teal-500"
                      >
